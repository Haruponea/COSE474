...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    def softmax(X):
      expX=np.exp(X)
      expXsum=np.sum(expX,axis=1, keepdims=True)
      result = expX/expXsum
      return result

    probabilities = softmax(scores)
    ansProbablities = probabilities[np.arange(N), y]
    NLL=-((np.sum(np.log(ansProbablities)))/N)

    weightSum= np.sum(W1**2) + np.sum(W2**2)
    regularizer = reg * weightSum
    ...
    dLdz2=softmax(scores)
    for i in range(N):
      j=y[i]
      dLdz2[i][j] -= 1
    dLdz2 /= N

    dLdz1 = dLdz2.dot(W2.T)
    a, b = dLdz1.shape
    for i in range(a):
      for j in range(b):
        if z1[i][j] < 0:
    ...
    grads['W2'] = p1.T.dot(dLdz2) + 2*reg*W2
    grads['b2'] = np.sum(dLdz2, axis=0)
    grads['W1'] = X.T.dot(dLdz1) + 2*reg*W1
    grads['b1'] = np.sum(dLdz1, axis=0)
